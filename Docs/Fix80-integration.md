...
- Dependency-aware backend scope is recorded in `Fix80-backend-deploy.json`: 48 functions and Firestore Rules. The unrelated `cleanupUnverifiedAccounts` schedule exists in base79 source but is absent from the live project and will not be newly activated by this rollout.
