 ...
 */
object PlatformRoleJournalCodec {
    const val MAX_ENTRIES = PlatformRoleRecovery.MAX_PENDING
    const val MAX_BYTES = 32_768
    private const val MAGIC = 0x5541524a
    private const val SCHEMA = 1

    private fun backendBinding(): String =
        PlatformRoleRecovery.hash(
            listOf(
                    "uac-platform-role-journal-v1",
    ...
    private fun validate(entries: List<PlatformRolePending>) {
        require(entries.size <= MAX_ENTRIES)
        entries.forEach(PlatformRoleRecovery::validate)
        require(
    ...
    fun encode(entries: List<PlatformRolePending>): ByteArray = checked {
        validate(entries)
        ...
        DataOutputStream(limited).use { out ->
            ...
            entries.forEach { entry ->
                ...
                text(entry.version.previousRole, 8)
    ...
    fun decode(bytes: ByteArray): List<PlatformRolePending> = checked {
        ...
        DataInputStream(ByteArrayInputStream(bytes)).use { input ->
            ...
            val entries =
                List(count) {
                    ...
                    val account = text()
                    val version = PlatformRoleVersion(text(512), text(), text(), text(8))
                    val action = PlatformRoleAction.valueOf(text(32))
                    val reason = text()
                    val operation = text(36)
                    val phase = PlatformRolePhase.valueOf(text(32))
...
class FilePlatformRoleJournal(
    ...
    private fun read(): List<PlatformRolePending> {
        requirePaths()
        if (!exists(base)) return emptyList()
        require(base.length() in 1..PlatformRoleJournalCodec.MAX_BYTES.toLong())
        val bytes =
            atomic.openRead().use { input ->
                ...
                while (true) {
                    ...
                    if (count < 0) break
                    require(count <= PlatformRoleJournalCodec.MAX_BYTES - output.size())
                    output.write(buffer, 0, count)
                }
                output.toByteArray()
            }
        return PlatformRoleJournalCodec.decode(bytes)
    }

    private fun write(entries: List<PlatformRolePending>) {
    ...
    override suspend fun pending(uid: String): List<PlatformRolePending> = locked {
        val owner = PlatformRoleRecovery.accountHash(uid)
    ...
    override suspend fun put(
    ...
    ): PlatformRolePending = locked {
        require(entry.accountHash == PlatformRoleRecovery.accountHash(uid))
        PlatformRoleRecovery.validate(entry)
        expected?.let {
            PlatformRoleRecovery.validate(it)
            require(
        ...
        if (old == null) require(entry.phase == PlatformRolePhase.PREPARED)
        else
            require(
                entry.copy(phase = old.phase, receipt = old.receipt) == old &&
                    (old.phase == PlatformRolePhase.PREPARED &&
                        entry.phase == PlatformRolePhase.DISPATCHED ||
                        old.phase == PlatformRolePhase.DISPATCHED &&
                            entry.phase == PlatformRolePhase.ACKNOWLEDGED)
            )
        write(all.filterNot { it == old } + entry)
        entry
    }

    override suspend fun clear(uid: String, expected: PlatformRolePending) = locked {
        require(expected.accountHash == PlatformRoleRecovery.accountHash(uid))
        PlatformRoleRecovery.validate(expected)
        val all = read()
        require(
            all.firstOrNull {
...
object LocalPlatformRoleJournal {
    @Volatile private var instance: PlatformRoleJournal? = null
    @Volatile private var boundDirectory: File? = null

    fun get(context: Context): PlatformRoleJournal =
        synchronized(this) {
            val app = context.applicationContext
            CompiledBackend.configuration.requireAndroidPackage(app.packageName)
            val directory = File(app.noBackupFilesDir, "platform-role-recovery").canonicalFile
            require(boundDirectory == null || boundDirectory == directory)
            instance
                ?: FilePlatformRoleJournal(directory).also {
