...
class UserStatusCallableProtocolTest {
    private val names =
        listOf("warnUser", "suspendUser", "banUser", "deactivateUser", "restoreUser")

    @Test
    fun fiveExactExistingStatusCallablesUseTheLocalTransportOnly() {
        names.forEach { name ->
            assertEquals(
    ...
    @Test
    fun evenRestoreAppendsRecordsAndIsNotIdempotent() {
        names.forEach { assertTrue(LocalCallableProtocol.nonIdempotent(it)) }
    }

    @Test
    fun noStatusActionReceivesTheLargerMediaBudgetOrDeletionDeadline() {
        names.forEach { name ->
    ...
    @Test
    fun statusTransportCannotSelectCloudForeignRegionOrAnotherPort() {
        names.forEach { name ->
